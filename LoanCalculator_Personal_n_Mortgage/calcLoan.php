...
<!--PHP code for calculation: 29-Nov-2016-->
<!--Go with: 28_11_2016_Loan_Interest_v14_latest.xlsx for test data-->
...
//#$str_time_next_payment for calculating $str_time_next_payment
...
//Input $str_next_payment: "1D", "12M", $start_date: '2016-01-31'
...
function calc_pmtd ($loan_amt, $periodic_interest, $num_periodic_pmts){
    $periodic_pmt_amt =
    ($loan_amt*(pow(1+$periodic_interest,$num_periodic_pmts)))*$periodic_interest/(pow(1+$periodic_interest,$num_periodic_pmts) -1)/(1+$periodic_interest);
    return $periodic_pmt_amt;
}
...
function calc_apr_new($selected_pmt_interval,
    ...
    //2.1 Consider the $points parameter -> to adjust interest
    ...
    if($loan_date <=$p0_date) {
        ...
        if($has_interest_on_odd_days_interest =="No"){
            ...
            $period_interest_pmt = $odd_days_interest+
                $interest_on_odd_days_interest_p0_p1+$interest_on_loan_amt_p0_p1+$pmt_of_points;
        }
...
function calc_pmt_periods_interest_only($loan_amt,$loan_date,$first_pmt_due,
                                        ...
                                        $num_days_in_year,$has_interest_on_odd_days_interest,
                                        $pmt_of_points){
    //This 2 dim-asso array holds data of Periods.
    $periods =array();
...
function display_user_input_customized($loan_type,$num_days_in_year,$quoted_apr,
                                       $loan_amt,$num_pmts_years,$num_pmts_months,
                                        $loan_date,$first_pmt_due,$selected_pmt_interval,
                                        $selected_comp_interval){
    $num_pmts = $num_pmts_years *12 + $num_pmts_months; //INPUT_4: 15

    echo "<h3>Inputs:</h3>" .
        "<table border='1'>
            <tr>
                <td>Loan type: " . $loan_type ."</td>
                <td></td>                                                           
            </tr>
            <tr>
                <td>Number of days in a year: " . $num_days_in_year ."</td>
                <td>Quoted Annual Percentage Rate APR(%): " . ($quoted_apr*100) . "</td>                
            </tr>
            <tr>
                <td>Loan Amount($): " . number_format($loan_amt,2) . "</td>
                <td>Number of payments: " . $num_pmts . "months(".$num_pmts_years."yrs +".$num_pmts_months."ms)</td>                
            </tr>
            <tr>
...
function calc_pmt_periodic_personal($loan_amt,$loan_date,$num_pmts,
                                    $first_pmt_due,$num_days_in_year,
                                    $apr_new,$periodic_interest_amt_std,
                                    $str_len_pmt_interval,$pmta_amt,$pmt_of_points){
    //This 2 dim-asso array holds data of Periods.
    $periods = array();
    ...
    $period_interest_reduction_std = $loan_amt/$num_pmts;
    ...
    $period_date = $first_pmt_due;
    $open_principal_bal = $end_principal_bal;
    $period_interest_pmt = $odd_days_interest + $periodic_interest_amt_std+$pmt_of_points;

    $period_reduction_principal = $period_interest_reduction_std;
    $period_pmt = $period_interest_pmt+ $period_reduction_principal;
    $end_principal_bal = $open_principal_bal - $period_reduction_principal;
    ...
    for ($i = 2; $i < $num_pmts + 1; $i++) {
        ...
        //Calculate the interest each period
        $period_interest_pmt =$periodic_interest_amt_std;
...
function calc_pmt_periods_mortgage($loan_amt,$loan_date,$first_pmt_due,$num_pmts,
                                   $apr_new,$num_days_in_year,$pmtd_amt,
                                   $str_len_pmt_interval,$periodic_interest_rate,$pmt_of_points){
    //This 2 dim-asso array holds data of Periods.
    $periods = array();
    ...
    //Odd-days (Loan date-> P1): Only affect on the first payment
    //Handle odd days interest on loan_amt for (Loan_date->P0)
    $odd_days = num_days_diff($loan_date, $first_pmt_due);//note: function always return >0
    $odd_days_interest = ($loan_amt * $apr_new / $num_days_in_year) * $odd_days;
    ...
    $open_principal_bal = $end_principal_bal;
    $period_interest_pmt = $odd_days_interest+$pmt_of_points;
    $period_reduction_principal = $pmtd_amt;
    $period_pmt = $period_interest_pmt+ $period_reduction_principal;
    $end_principal_bal = $open_principal_bal - $period_reduction_principal;
    ...
    for ($i = 2; $i < $num_pmts + 1; $i++) {
        ...
        //Calculate the interest each period
        $period_interest_pmt =$periodic_interest_rate*$open_principal_bal;

        $period_reduction_principal = $pmtd_amt - $period_interest_pmt;
        $period_pmt = $pmtd_amt;
        $end_principal_bal = $open_principal_bal - $period_reduction_principal;
...
function calc_loan_personal($loan_amt,$loan_date,$num_days_in_year,$selected_pmt_interval,
                            $selected_comp_interval,$quoted_apr,$points,$num_pmts,
                            $first_pmt_due,$intervals_table){
    //Update #days_in_year, that affects the daily interest
    $interval_lookup = '1 day';
    $r = update_num_days_in_year($intervals_table, $interval_lookup, $num_days_in_year);
    ...
    //Calculate payment
    $total_pmt_std = $loan_amt * pow(1 + $periodic_interest_rate, $num_pmts);
    $total_interest_pmt_std = $total_pmt_std - $loan_amt;

    //Calculate periodic_pmt_amt
    $pmta_amt = $total_pmt_std / $num_pmts;
    $periodic_interest_amt_std = $total_interest_pmt_std / $num_pmts;

    //Payment for Points
    $pmt_of_points = $loan_amt * $points;

    //Detail each pmt period
    $detailed_periods = calc_pmt_periodic_personal($loan_amt,$loan_date,$num_pmts,
        $first_pmt_due,$num_days_in_year,
        $apr_new,$periodic_interest_amt_std,
        $str_len_pmt_interval,$pmta_amt,$pmt_of_points);
...
function calc_loan_mortgage($loan_amt,$loan_date,$num_days_in_year,$selected_pmt_interval,
                            $intervals_table,$selected_comp_interval,$quoted_apr,$points,
                            $num_pmts,$first_pmt_due){
    //Update #days_in_year, that affects the daily interest
    $interval_lookup = '1 day';
    $r = update_num_days_in_year($intervals_table, $interval_lookup, $num_days_in_year);
    ...
    //Calculate periodic_pmt_amt
    $pmta_amt = calc_pmtd($loan_amt, $periodic_interest_rate, $num_pmts);

    //Payment for Points
    $pmt_of_points = $loan_amt * $points;

    //Detail each pmt period
    $detailed_periods = calc_pmt_periods_mortgage($loan_amt,$loan_date,$first_pmt_due,$num_pmts,
        $apr_new,$num_days_in_year,$pmta_amt,
        $str_len_pmt_interval,$periodic_interest_rate,$pmt_of_points);
    $periods = $detailed_periods['periods'];
    $total_actual_pmt = $detailed_periods['total_actual_pmt'];
...
function calcLoanPayments($loan_type,$loan_amt, $quoted_apr, $num_pmts,
    ...
    if ($loan_type == 'personal') {
        $results = calc_loan_personal($loan_amt,$loan_date,$num_days_in_year,$selected_pmt_interval,
            $selected_comp_interval,$quoted_apr,$points,$num_pmts,
            $first_pmt_due,$intervals_table);

    }elseif ($loan_type=='mortgage') {
        $results = calc_loan_mortgage($loan_amt,$loan_date,$num_days_in_year,$selected_pmt_interval,
                    $intervals_table,$selected_comp_interval,$quoted_apr,$points,
                    $num_pmts,$first_pmt_due);


    }elseif ($loan_type == 'bond') {
        //Handle each Method
        switch ($amort_method) {
            case 'fixed_pmt':
            ...
            default:
                echo "Upcoming amortization methods!!!";
        }//end switch statement amortization method
    }
...
if(isset($_POST['loan_type']) && $_POST['loan_type'] !=""&&
    isset($_POST['num_days_in_year'])&& $_POST['num_days_in_year']!=""&&
    isset($_POST['quoted_apr'])&& $_POST['quoted_apr']!=""&&
    isset($_POST['points']) && $_POST['points']!=""&&
    isset($_POST['loan_amt']) && $_POST['loan_amt']!=""&&

    isset($_POST['num_pmts_years']) && $_POST['num_pmts_years']!=""&
    isset($_POST['num_pmts_months']) && $_POST['num_pmts_months']!=""&
    ...
    //note: amort_method exists here just for sake of not having to alter the interface method calcLoanPayments()

    //===================================USER INPUTs================================================
    $loan_type = $_POST['loan_type'];//Personal Loan
    ...
    $num_pmts_years = $_POST['num_pmts_years'];
    $num_pmts_months = $_POST['num_pmts_months'];
    $num_pmts = $num_pmts_years *12 + $num_pmts_months; //INPUT_4: 15
    ...
    // Display user inputs
    display_user_input_customized($loan_type,$num_days_in_year,$quoted_apr,
        $loan_amt,$num_pmts_years,$num_pmts_months,
        $loan_date,$first_pmt_due,$selected_pmt_interval,
        $selected_comp_interval);
    //===================================//END USER INPUTS================================================
    ...
    $calculation_result = json_decode(calcLoanPayments($loan_type,$loan_amt, $quoted_apr, $num_pmts,
                                $loan_date, $selected_pmt_interval,
                                $points, $amort_method, $num_days_in_year,
                                $first_pmt_due,$selected_comp_interval,
                                $intervals_table,$has_interest_on_odd_days_interest),true); //true: to return PHP array, otherwise, it will be an object.
    ...
    render_result_at_backend($periods, "Amortization Schedule Table<br/>");
